...
class AdapterMLP(nn.Module):
    def __init__(self, config, dim, mlp_ratio=4., dropout=0.):
        super().__init__()
        ...
        # Adapter-specific layers
        self.adaptmlp = Adapter(config, dropout=dropout, bottleneck=config.ffn_num,
    ...
    def forward(self, x):
        ...
        if self.config.ffn_option == 'sequential':
            ...
            adapt_x = self.adaptmlp(residual, add_residual=False)
            x = x + adapt_x
        else:
...
class VisionTransformer2(VisionTransformer):
    ...
    def forward_block_post(self, ii, x, x_t, B):
        ...
        # x_t = self.temporal_att_post[ii](x_t) # 上采样 + GELU
        # x = x + nn.functional.tanh(self.all_gate[ii])* x_t.unsqueeze(2).view(B, -1, 768)

        print("ii: ","  all_gate[ii]:",self.all_gate[ii],"  tanh  ",nn.functional.tanh(self.all_gate[ii]))

        # if ii % 6 == 0:
        #     prompts_progr = self.learnable_prompts_progr[ii//6].expand(B, -1, -1) #torch.Size([16, 3, 768])
        ...
        #     # 取出需要修改的部分，并做加法操作，生成新的张量
        #     x_slice = x[:, self.n_seq + 1 + ii // 6 * self.n_progr:self.n_seq + 1 + (ii // 6 + 1) * self.n_progr, :]
        #     new_x_slice = x_slice + prompts_progr
        #
        #     # 用修改后的新张量替换原来的切片
        #     x = torch.cat((x[:, :self.n_seq + 1 + ii // 6 * self.n_progr, :],  # 保留切片前部分
        #                    new_x_slice,  # 插入新计算出来的切片
        #                    x[:, self.n_seq + 1 + (ii // 6 + 1) * self.n_progr:, :]), dim=1)  # 保留切片后部分
        #
        #      #   197+ 3 ： 197 + 6
